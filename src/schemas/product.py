...
# ============================================
# ✅ ИСПРАВЛЕНО: ProductCatalogItem по спецификации OpenAPI
# ============================================

class ProductCatalogItem(BaseModel):
    """
    Товар в публичном каталоге для B2C (service-to-service).
    
    Соответствует ProductPublicShortResponse в b2b/openapi.yaml:1395
    НЕ содержит cost_price и reserved_quantity (IDOR-защита).
    """
    id: UUID
    title: str
    slug: str                          # ✅ ДОБАВЛЕНО (required по OpenAPI)
    seller_id: UUID                    # ✅ ДОБАВЛЕНО (required по OpenAPI)
    category_id: UUID                  # ✅ ДОБАВЛЕНО (вместо вложенного category)
    min_price: int                     # ✅ ДОБАВЛЕНО (required по OpenAPI)
    has_stock: bool = True
    images: List[Any] = []
    created_at: datetime               # ✅ ДОБАВЛЕНО (required по OpenAPI)
