...
class ProductService:
    ...
    # ============================================
    # ✅ ИСПРАВЛЕНО: _format_for_catalog по спецификации OpenAPI
    # ============================================
    def _format_for_catalog(self, product: Product) -> dict:
        """
        Format product for B2C catalog — no cost_price, no reserved_quantity.
        
        Соответствует ProductPublicShortResponse в b2b/openapi.yaml:1395
        """
        # Минимальная цена среди активных SKU
        skus = product.skus or []
        active_skus = [s for s in skus if s.get("active_quantity", 0) > 0]

        if active_skus:
            min_price = min(s.get("price", 0) for s in active_skus)
            has_stock = True
        else:
            min_price = 0
            has_stock = False
        ...
        return {
            "id": product.id,
            "title": product.title,
            "slug": product.slug,                          # ✅ ДОБАВЛЕНО
            "seller_id": product.seller_id,                # ✅ ДОБАВЛЕНО
            "category_id": product.category_id,            # ✅ ДОБАВЛЕНО (вместо category)
            "min_price": min_price,                        # ✅ ДОБАВЛЕНО
            "has_stock": has_stock,                        # ✅ ДОБАВЛЕНО
            "images": images,
            "created_at": product.created_at,              # ✅ ДОБАВЛЕНО
        }
