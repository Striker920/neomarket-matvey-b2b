...
class InvoiceService:
    ...
    def create_invoice(self, seller_id: str, items: list) -> Invoice:
        ...
        2. SKU принадлежит seller_id (ownership check)
        3. Товар SKU имеет статус MODERATED
        
        Статус накладной после создания: CREATED (по спецификации OpenAPI)
        """
        if not items:
        ...
        for item in items:
            ...
            # Проверка статуса товара (должен быть MODERATED)
        ...
        invoice = Invoice(
            id=str(uuid4()),
            seller_id=seller_id,
            status=InvoiceStatus.CREATED,  # По спецификации OpenAPI
