...
class TestB2CCatalog:
    ...
    def test_catalog_returns_moderated_in_stock_products(self, client, db_session):
        ...
        # ✅ Проверяем обязательные поля по OpenAPI
        item = next(p for p in data["items"] if p["title"] == "iPhone 15")
        assert "slug" in item
        assert "seller_id" in item
        assert "category_id" in item
        assert "min_price" in item
        assert "created_at" in item
        # ✅ category_id — скалярный UUID, не вложенный объект
    ...
    def test_catalog_response_has_no_cost_price(self, client, db_session):
        ...
        for item in data["items"]:
            if item["title"] == "Secret Product":
                # ✅ IDOR-защита: нет cost_price и reserved_quantity
                assert "cost_price" not in item
                assert "reserved_quantity" not in item
                # ✅ Проверяем, что min_price есть
                assert "min_price" in item
                break
    ...
    def test_batch_ids_returns_visible_subset(self, client, db_session):
        ...
        assert response.status_code == 200
        data = response.json()
        returned_ids = [str(item["id"]) for item in data["items"]]
    ...
    def test_filter_by_category_id(self, client, db_session):
        """Фильтрация по category_id (переименованный параметр)."""
        cat_id = str(uuid4())
        other_cat_id = str(uuid4())

        product_in_cat = Product(
            id=str(uuid4()),
            seller_id=str(uuid4()),
            category_id=cat_id,
            title="In Category",
            ...
            deleted=False,
            blocked=False,
            images=[{"url": "/s3/in.jpg", "ordering": 0}],
            characteristics=[],
            skus=[{
                "id": str(uuid4()),
                "sku_code": "SKU-CAT",
        ...
        product_other_cat = Product(
            ...
            deleted=False,
            blocked=False,
            images=[{"url": "/s3/other.jpg", "ordering": 0}],
            characteristics=[],
            skus=[{
                "id": str(uuid4()),
                "sku_code": "SKU-OTHER",
        ...
        db_session.add_all([product_in_cat, product_other_cat])
        db_session.commit()

        response = client.get(
            f"/api/v1/public/products?category_id={cat_id}",
            headers={"X-Service-Key": settings.B2C_SERVICE_KEY}
        )
        ...
        assert str(data["items"][0]["category_id"]) == cat_id